...
## Changes in the version on 2026/3/31:
## 1. Allow the return of predicted 1-year risk (set return_1yr_risk = TRUE)
## 2. Allow the users to choose between average prediction across 30 models and the prediction based on average coefficients (use_avg_coef = FALSE for the former and use_avg_coef = TRUE for the latter)
## 3. Allow the return of predicted 1-year risk of competing-risk event (set return_compete_risk = TRUE)

{
  ...
  # load(paste0(path,"5yr_model_needed_materials.RData"))
  ...
  ## Read in averaged coefficients if avg_coef==TRUE
  if(use_avg_coef==TRUE){
    # library(openxlsx)
    # avg_coef_df = read.xlsx(xlsxFile = "average_coef.xlsx")
    avg_coef = avg_coef_df$coef
    names(avg_coef) = avg_coef_df$parameter
    # avg_coef_competingRisk_df = read.xlsx(xlsxFile = "average_coef_competingRisk.xlsx")
    avg_coef_competingRisk = avg_coef_competingRisk_df$coef
    names(avg_coef_competingRisk) = avg_coef_competingRisk_df$parameter
  }
  ...
  if(use_avg_coef == FALSE){
    ...
    density_c:racenci_new + mossinceprvsurvmm_cat:racenci_new")
      ...
      library(glmnet)
      source("funcs_normalizeRisk_deid.R")
  ...
  ### === Calculate predicted risk with average coefficients === ###
  else{
    risk_pr_avgcoef = sapply(1, simplify=FALSE,function(model_round){
      print("Risk prediction with average coefficients")
      imp_act_tmp = data_df_new
      w = do.call(rbind,weight_list_deid[imp_act_tmp$racenci_new])
    ...
    density_c:racenci_new + mossinceprvsurvmm_cat:racenci_new")
      ...
      source("funcs_normalizeRisk_deid.R")
      input_x_index = model.matrix(formula_glmnet,data=imp_act_tmp)[,-1]
      risk_pr_1yr_index_w = normalizeRisk_siteDxyear_avgCoef(avg_coef = avg_coef,, 
                                                             data = input_x_index, 
                                                             weights=w,
                                                             designMx_siteDxyear = uniq_comb_glmnet_input_deid)
      risk_pr_1yr_compete_index_w = normalizeRisk_siteDxyear_avgCoef(avg_coef = avg_coef_competingRisk,
                                                                     data = input_x_index,
                                                                     weights=w,
                                                                     designMx_siteDxyear = uniq_comb_glmnet_input_deid)
      print("risk at index exam done!")
      ...
      risk_pr_1yr_4fu_w = sapply(1:4,simplify=FALSE,function(fu_yr){
        ...
        input_x_fu = model.matrix(formula_glmnet,data=cbind(input_x_constant,input_x_assign,input_x_varying))[,-1]
        risk_pr_1yr_fu = normalizeRisk_siteDxyear_avgCoef(avg_coef = avg_coef,
                                                          data = input_x_fu,
                                                          weights=w,
                                                          designMx_siteDxyear = uniq_comb_glmnet_input_deid)
        return(risk_pr_1yr_fu)
      ...
      risk_pr_1yr_compete_4fu_w = sapply(1:4,simplify=FALSE,function(fu_yr){
        ...
        input_x_fu = model.matrix(formula_glmnet,data=cbind(input_x_constant,input_x_assign,input_x_varying))[,-1]
        risk_pr_1yr_fu = normalizeRisk_siteDxyear_avgCoef(avg_coef = avg_coef_competingRisk,
                                                  data = input_x_fu,
                                                  weights=w,
                                                  designMx_siteDxyear = uniq_comb_glmnet_input_deid)
        return(risk_pr_1yr_fu)
    ...
    ### === Get 1-year predictions from the model with average coefficients === ###
    ## 1yr risk of primary event at 5 rounds, race-stratified weighted normalization ==
    risk_pr_mx_lasso_1yr_index_w = do.call(cbind,sapply(risk_pr_avgcoef, function(res) res[["primary_w"]][1]))
    risk_pr_mx_lasso_1yr_fu1_w = do.call(cbind,sapply(risk_pr_avgcoef, function(res) res[["primary_w"]][2]))
    risk_pr_mx_lasso_1yr_fu2_w = do.call(cbind,sapply(risk_pr_avgcoef, function(res) res[["primary_w"]][3]))
    risk_pr_mx_lasso_1yr_fu3_w = do.call(cbind,sapply(risk_pr_avgcoef, function(res) res[["primary_w"]][4]))
    risk_pr_mx_lasso_1yr_fu4_w = do.call(cbind,sapply(risk_pr_avgcoef, function(res) res[["primary_w"]][5]))
    risk_pr_lasso_1yr_index_w = rowMeans(risk_pr_mx_lasso_1yr_index_w)
    risk_pr_lasso_1yr_fu1_w = rowMeans(risk_pr_mx_lasso_1yr_fu1_w)
    risk_pr_lasso_1yr_fu2_w = rowMeans(risk_pr_mx_lasso_1yr_fu2_w)
    risk_pr_lasso_1yr_fu3_w = rowMeans(risk_pr_mx_lasso_1yr_fu3_w)
    risk_pr_lasso_1yr_fu4_w = rowMeans(risk_pr_mx_lasso_1yr_fu4_w)
    # risk_pr_lasso_1yr_index_w = risk_pr_avgcoef[["primary_w"]][1]
    # risk_pr_lasso_1yr_fu1_w = risk_pr_avgcoef[["primary_w"]][2]
    # risk_pr_lasso_1yr_fu2_w = risk_pr_avgcoef[["primary_w"]][3]
    # risk_pr_lasso_1yr_fu3_w = risk_pr_avgcoef[["primary_w"]][4]
    # risk_pr_lasso_1yr_fu4_w = risk_pr_avgcoef[["primary_w"]][5]
    
    ## 1yr risk of competing event at 5 rounds, race-stratified weighted normalization ==
    risk_pr_mx_lasso_1yr_compete_index_w = do.call(cbind,sapply(risk_pr_avgcoef, function(res) res[["compete_w"]][1]))
    risk_pr_mx_lasso_1yr_compete_fu1_w = do.call(cbind,sapply(risk_pr_avgcoef, function(res) res[["compete_w"]][2]))
    risk_pr_mx_lasso_1yr_compete_fu2_w = do.call(cbind,sapply(risk_pr_avgcoef, function(res) res[["compete_w"]][3]))
    risk_pr_mx_lasso_1yr_compete_fu3_w = do.call(cbind,sapply(risk_pr_avgcoef, function(res) res[["compete_w"]][4]))
    risk_pr_mx_lasso_1yr_compete_fu4_w = do.call(cbind,sapply(risk_pr_avgcoef, function(res) res[["compete_w"]][5]))
    risk_pr_lasso_1yr_compete_index_w = rowMeans(risk_pr_mx_lasso_1yr_compete_index_w)
    risk_pr_lasso_1yr_compete_fu1_w = rowMeans(risk_pr_mx_lasso_1yr_compete_fu1_w)
    risk_pr_lasso_1yr_compete_fu2_w = rowMeans(risk_pr_mx_lasso_1yr_compete_fu2_w)
    risk_pr_lasso_1yr_compete_fu3_w = rowMeans(risk_pr_mx_lasso_1yr_compete_fu3_w)
    ...
    # risk_pr_lasso_1yr_compete_index_w = risk_pr_avgcoef[["compete_w"]][1]
    # risk_pr_lasso_1yr_compete_fu1_w = risk_pr_avgcoef[["compete_w"]][2]
    # risk_pr_lasso_1yr_compete_fu2_w = risk_pr_avgcoef[["compete_w"]][3]
    # risk_pr_lasso_1yr_compete_fu3_w = risk_pr_avgcoef[["compete_w"]][4]
    # risk_pr_lasso_1yr_compete_fu4_w = risk_pr_avgcoef[["compete_w"]][5]
  }
  ...
  if(nrow(data_df_new)==length(risk_pr_lasso_5yr_w)){
    if(return_1yr_risk==FALSE & return_compete_risk==FALSE) data_output = data_df_new %>% mutate(risk_5yr = risk_pr_lasso_5yr_w)
    if(return_1yr_risk==TRUE & return_compete_risk==FALSE) data_output = data_df_new %>% mutate(risk_5yr = risk_pr_lasso_5yr_w,
                                                                                               risk_1yr = risk_pr_lasso_1yr_index_w)
    if(return_1yr_risk==FALSE & return_compete_risk==TRUE) data_output = data_df_new %>% mutate(risk_5yr = risk_pr_lasso_5yr_w,
                                                                                               risk_1yr_cr = risk_pr_lasso_1yr_compete_index_w)
    if(return_1yr_risk==TRUE & return_compete_risk==TRUE) data_output = data_df_new %>% mutate(risk_5yr = risk_pr_lasso_5yr_w,
                                                                                                risk_1yr = risk_pr_lasso_1yr_index_w,
                                                                                                risk_1yr_cr = risk_pr_lasso_1yr_compete_index_w)
  ...
  return(predicted_risk = data_output)
}
